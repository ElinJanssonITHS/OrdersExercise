...
public record LineItem
{
    ...
    public LineItem(int orderId, int id, string product, int quantity, double price, double vat)
    {
        try
        {
            ...
            else if (id < 1) throw new ArgumentException("The Id must be grater than 0.");
            else if (product.Equals(string.Empty) || product == default) throw new ArgumentException("The product name cannot be empty.");
            else if (quantity < 1) throw new ArgumentException("The quantity must be grater than 0");
            else if (price <= 0) throw new ArgumentException("The price must be grater than or equal to 0.");
            else if (vat < 0) throw new ArgumentException("The VAT must be grater than 0");
